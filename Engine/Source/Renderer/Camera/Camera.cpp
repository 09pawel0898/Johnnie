...
namespace Engine
{
	Camera::Camera(float FieldOfView, float AspectRatio, float NearClip, float FarClip, glm::vec3 SpawnLocation)
	...
	void Camera::UpdateWorldCamera(void)
	{

		m_Target = { 0.f,-0.5f,0.f };

		m_Forward	= glm::normalize(m_Target - m_Location);
	...
	void Camera::UpdateView(void)
	{
		...
		m_ViewMat = glm::lookAt(m_Location, m_Target, m_Up);
	}
}